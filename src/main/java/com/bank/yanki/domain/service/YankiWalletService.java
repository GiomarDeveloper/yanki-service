...
public class YankiWalletService {
    ...
    private final ConcurrentHashMap<String, PendingTransaction> pendingTransactions = new ConcurrentHashMap<>();
        ...
        return cacheService.getCachedWallet(phoneNumber)
                ...
                .switchIfEmpty(
                        ...
                        walletRepository.findByPhoneNumber(phoneNumber)
                                .switchIfEmpty(Mono.error(new WalletNotFoundException("Wallet not found with phone number: " + phoneNumber)))
                                .flatMap(wallet ->
    ...
    public Mono<Transaction> processPayment(String fromPhoneNumber, String toPhoneNumber,
        ...
        return findByPhoneNumber(fromPhoneNumber)
                .flatMap(fromWallet -> {
                    if (fromWallet.getAssociatedCreditId() != null && !fromWallet.getAssociatedCreditId().isEmpty()) {
                        // Para tarjetas, iniciamos validación y retornamos una transacción pendiente
                        return validateCreditBalanceViaKafka(fromWallet, toPhoneNumber, paymentAmount, description)
                ...
                .doOnSuccess(transaction -> {
                    if (transaction.getStatus() == TransactionStatusEnum.COMPLETED) {
                        log.info("Payment processed immediately: {} from {} to {}", amount, fromPhoneNumber, toPhoneNumber);
                            ...
                            .then(Mono.defer(() -> {
                                ...
                                if (!toWalletHasCard) {
                                    ...
                                    TransactionPaymentRequestEvent paymentEvent = TransactionPaymentRequestEvent.builder()
                                            ...
                                            .amount(paymentAmount)
                                            .description("Yanki Transfer from " + fromWallet.getPhoneNumber() + ": " + description)
    ...
    private Mono<Void> sendKafkaEvents(YankiWallet fromWallet, YankiWallet toWallet,
        ...
        if (fromWallet.getAssociatedCreditId() != null && !fromWallet.getAssociatedCreditId().isEmpty()) {
            TransactionConsumptionRequestEvent consumptionEvent = TransactionConsumptionRequestEvent.builder()
        ...
        else if (toWallet.getAssociatedCreditId() != null && !toWallet.getAssociatedCreditId().isEmpty()) {
            TransactionPaymentRequestEvent paymentEvent = TransactionPaymentRequestEvent.builder()
                    .paymentId(UUID.randomUUID().toString())
                    .creditId(fromWallet.getAssociatedCreditId()) // Del REMITENTE (FROM) - pero para SUMAR al TO
                    .amount(paymentAmount)
                    .description("Yanki Transfer from " + toWallet.getPhoneNumber() + ": " + description)
                    .customerId(fromWallet.getDocumentNumber())
                    .paymentDate(LocalDateTime.now())
        ...
        return Mono.fromRunnable(() -> {
            ...
            if (responseEvent.getIsValid()) {
                ...
                proceedWithPayment(
                    ...
                    log.info("Payment completed after credit validation - TransactionId: {}", transaction.getId());
